/*function ListSelect(options)
{
	...
	elem.onmclick = function(e)
	{
		var li = e.target.closest("li");

		if(!li) return;

		if(e.metaKey || e.ctrlKey) // для Mac проверяем Cmd, т.к. Ctrl + click там к
		{
			toggleSelect(li);
		}else if(e.shiftKey)
		{
	...
	function selectFromLast(target)
	{
      ...
      // это особый случай
		...
		var isLastClickBefore = startElem.compareDocumentPosition(target) & 4;
		if(isLastClickBefore)
		{
			for(let elem = startElem; elem != target; elem = elem.nextElementSibiling)
			{
